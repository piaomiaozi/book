...
public class SentenceDetectionTest extends TamingTextTestJ4 {
	...
	public void testBreakIterator() {
		// <start id="sentDetect"/>
		BreakIterator sentIterator = BreakIterator
	...
	public void testOpenNLP() throws Exception {
		...
		String testString = "This is a sentence. It has fruits, vegetables,"
				+ " etc. but does not have meat. Mr. Smith went to Washington.";
		String[] result = detector.sentDetect(testString); // <co
															// id="openSentDetect.co.run"/>
		for (int i = 0; i < result.length; i++) {
			System.out.println("Sentence: " + result[i]);
		}
		/*
		 * <calloutlist> <callout
		 * arearefs="openSentDetect.co.detect"><para>Create the
		 * <command>SentenceDetector</command> with the en-sent.bin
		 * model</para></callout> <callout
		 * arearefs="openSentDetect.co.run"><para>Invoke the detection
		 * process</para></callout> </calloutlist>
		 */
		// <end id="openSentDetect"/>
		assertTrue("result Size: " + result.length + " is not: " + 3,
				result.length == 3);
	}
