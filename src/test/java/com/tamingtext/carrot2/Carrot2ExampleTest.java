...
public class Carrot2ExampleTest extends TamingTextTestJ4 {
	...
	public void testSimple() throws Exception {
		...
		final Controller controller = ControllerFactory.createSimple();// <co
																		// id="crt2.controller.creation"/>
		documents = new ArrayList<Document>();
		for (int i = 0; i < titles.length; i++) {
			Document doc = new Document(titles[i], snippets[i], "file://foo_"
					+ i + ".txt");
			documents.add(doc);
		}
		final ProcessingResult result = controller.process(documents,
				"red fox", LingoClusteringAlgorithm.class);// <co
															// id="crt2.process"/>
		displayResults(result);// <co id="crt2.print"/>

		/*
		 * <calloutlist> <callout
		 * arearefs="crt2.controller.creation"><para>Create the
		 * <classname>IController</classname></para></callout> <callout
		 * arearefs="crt2.process"><para>Cluster the documents</para></callout>
		 * <callout arearefs="crt2.print"><para>Print out the
		 * clusters</para></callout> </calloutlist>
		 */
		// <end id="crt2.simple"/>
	}
