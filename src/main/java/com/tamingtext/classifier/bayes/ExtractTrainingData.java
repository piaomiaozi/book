...
/**
 * A utility to extract training data from a Lucene index using document term
 * vectors to recreate the list of terms found in each document. Writes output
 * in Mahout Bayes classifier input format
 */
public class ExtractTrainingData {

	private static final Logger log = LoggerFactory
			.getLogger(ExtractTrainingData.class);
	...
	public static void main(String[] args) {
		...
		Option inputOpt = obuilder
				...
				.withRequired(true)
				.withArgument(
						abuilder.withName("dir").withMinimum(1).withMaximum(1)
								.create())
				.withDescription("Lucene index directory containing input data")
				.withShortName("d").create();

		Option categoryOpt = obuilder
				.withLongName("categories")
				.withRequired(true)
				.withArgument(
						abuilder.withName("file").withMinimum(1).withMaximum(1)
								.create())
		...
		Option outputOpt = obuilder
				...
				.withDescription("Output directory").withShortName("o")
				.create();

		Option categoryFieldsOpt = obuilder
				.withLongName("category-fields")
				.withRequired(true)
				.withArgument(
						abuilder.withName("fields").withMinimum(1)
								.withMaximum(1).create())
				.withDescription(
						"Fields to match categories against (comma-delimited)")
				.withShortName("cf").create();

		Option textFieldsOpt = obuilder
				.withLongName("text-fields")
				.withRequired(true)
				.withArgument(
						abuilder.withName("fields").withMinimum(1)
								.withMaximum(1).create())
				.withDescription(
						"Fields from which to extract training text (comma-delimited)")
				.withShortName("tf").create();

		Option useTermVectorsOpt = obuilder
				.withLongName("use-term-vectors")
				.withDescription(
						"Extract term vectors containing preprocessed data "
		...
		Group group = gbuilder.withName("Options").withOption(inputOpt)
				.withOption(categoryOpt).withOption(outputOpt)
				.withOption(categoryFieldsOpt).withOption(textFieldsOpt)
		...
		try {
			...
			File categoryFile = new File(cmdLine.getValue(categoryOpt)
					.toString());

			if (!categoryFile.isFile()) {
				throw new IllegalArgumentException(categoryFile
						+ " does not exist or is not a directory");
			}
			...
			Collection<String> categoryFields = stringToList(cmdLine.getValue(
					categoryFieldsOpt).toString());

			if (categoryFields.size() < 1) {
				throw new IllegalArgumentException(
						"At least one category field must be spcified.");
			}

			Collection<String> textFields = stringToList(cmdLine.getValue(
					textFieldsOpt).toString());

			if (categoryFields.size() < 1) {
				throw new IllegalArgumentException(
						"At least one text field must be spcified.");
			}

			boolean useTermVectors = cmdLine.hasOption(useTermVectorsOpt);

			extractTraininingData(inputDir, categoryFile, categoryFields,
					textFields, outputDir, useTermVectors);
	 ...
	 * <p>
	 * Iterates over documents in the lucene index, the values in the
	 * categoryFields are inspected and if found to contain any of the strings
	 * found in the category file, a training data item will be emitted,
	 * assigned to the matching category and containing the terms found in the
	 * fields listed in textFields. Output is written to the output directory
	 * with one file per category.
	 * <p>
	 * The category file contains one line per category, each line contains a
	 * number of whitespace delimited strings. The first string on each line is
	 * the category name, while subsequent strings will be used to identify
	 * documents that belong in that category.
	 * <p>
	 * 'Technology Computers Macintosh' will cause documents that contain either
	 * 'Technology', 'Computers' or 'Machintosh' in one of their categoryFields
	...
	public static void extractTraininingData(File indexDir, File categoryFile,
			Collection<String> categoryFields, Collection<String> textFields,
			File outputDir, boolean useTermVectors) throws IOException {
		...
		for (int i = 0; i < max; i++) {
			if (!reader.isDeleted(i)) {
				...
				// determine whether any of the fields in this document contain
				// a
				...
				// append the terms from each of the textFields to the training
		...
		if (log.isInfoEnabled()) {
			...
			while (!keyList.isEmpty()) {
				...
				b.append(categoryCounts.get(key)).append('\t').append(key)
						.append('\n');
			}
			log.info(b.toString());
		}
	...
	/**
	 * Read the category file from disk, see
	 * {@link #extractTraininingData(File, File, Collection, Collection, File)}
	 ...
	 * @throws IOException
	 */
	public static Map<String, List<String>> readCategoryFile(File categoryFile)
			throws IOException {
		Map<String, List<String>> categoryMap = new HashMap<String, List<String>>();
		BufferedReader rin = new BufferedReader(new InputStreamReader(
				new FileInputStream(categoryFile), "UTF-8"));
		String line;
	...
	/**
	 * Obtain a writer for the training data assigned to the the specified
	 * category.
	 * <p>
	 * Maintains an internal hash of writers used for a category which must be
	 * closed by {@link #closeWriters()}.
	 * <p>
	 * 
	 * @param outputDir
	 * @param category
	 ...
	 * @throws IOException
	 */
	protected static PrintWriter getWriterForCategory(File outputDir,
			String category) throws IOException {
		PrintWriter out = trainingWriters.get(category);
		if (out == null) {
			out = new PrintWriter(new OutputStreamWriter(new FileOutputStream(
					new File(outputDir, category))));
	...
	/**
	 * Append the contents of the specified termVector to a buffer containing a
	 * list of terms
	 * 
	 * @param buf
	 * @param tv
