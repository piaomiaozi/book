...
public class BayesUpdateRequestProcessor extends UpdateRequestProcessor {
	...
	public BayesUpdateRequestProcessor(ClassifierContext ctx,
			Analyzer analyzer, String inputField, String outputField,
			String defaultCategory, UpdateRequestProcessor next) {
		super(next);
		this.ctx = ctx;
	...
	public String[] tokenizeField(String fieldName, SolrInputField field)
			throws IOException {
		if (field == null)
			return new String[0];
		...
		TokenStream ts = analyzer.tokenStream(inputField, new StringReader(
				input));
