...
public class ClassifyDocument {
	...
	public static void main(String[] args) {
		...
		Option modelOpt = obuilder
				...
				.withRequired(true)
				.withArgument(
						abuilder.withName("model").withMinimum(1)
								.withMaximum(1).create())
		...
		try {
			...
			if (!modelDir.isDirectory()) {
				throw new IllegalArgumentException(modelDir
			...
			TokenStream ts = analyzer.tokenStream(null, new InputStreamReader(
			...
			ClassifierResult[] cr = ctx
					.classifyDocument(document, "unknown", 5);
