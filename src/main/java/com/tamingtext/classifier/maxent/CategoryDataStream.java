...
public class CategoryDataStream implements ObjectStream<DocumentSample> {
	...
	public static final Logger log = org.slf4j.LoggerFactory
			.getLogger(CategoryDataStream.class);
	...
	public CategoryDataStream(File[] inputFiles, String encoding,
			Tokenizer tokenizer) {
	...
	public CategoryDataStream(String fileName, String encoding,
			Tokenizer tokenizer) {
	 ...
	 * line of training data, if we run out of lines in one file, move on to the
	 * next. If we are out of files, line will remain null when this method
	 * returns.
	 * 
	 * @throws RuntimeException
	 *             if there's a problem reading any of the input files.
	 */
		...
		try {
			while (line == null) {
				if (reader == null) {
					...
					if (inputFilesIndex >= inputFiles.length)
						break;

					// open the next file.
					currentFile = inputFiles[inputFilesIndex];
					reader = new BufferedReader(new InputStreamReader(
							new FileInputStream(currentFile), encoding));
				}
			...
			throw new RuntimeException("Error reading input from: "
					+ currentFile, e);
		}
	}
	...
	public DocumentSample read() {
		...
		String[] tokens = tokenizer.tokenize(document); // <co
														// id="mee.train.tok"/>
		return new DocumentSample(category, tokens); // <co
														// id="mee.train.sample"/>
	}
	/*
	 * <calloutlist> <callout arearefs="mee.train.read">Read a line training
	 * data</callout> <callout arearefs="mee.train.cat">Extract
	 * category</callout> <callout arearefs="mee.train.tok">Tokenize
	 * content</callout> <callout arearefs="mee.train.tok">Create
	 * sample</callout> </calloutlist>
	 */
