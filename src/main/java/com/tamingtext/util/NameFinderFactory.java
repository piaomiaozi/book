...
public class NameFinderFactory {
	 ...
	 * or, reads the <code>model.dir</code> system property in order to
	 * determine if the <code>modelDirectory</code> is <code>null</code> or
	 * empty.
	 * 
	 * @param language
	...
	/**
	 * Load name finder models based upon models for the specified language in
	...
	protected void loadNameFinders(String language, String modelDirectory)
		...
		for (int fi = 0; fi < models.length; fi++) {
			modelFile = models[fi];
			modelNames[fi] = modelNameFromFile(language, modelFile); // <co
																		// id="nfe.modelname"/>
		...
		/*
		 * <calloutlist> <callout arearefs="nfe.findmodels">Find
		 * Models</callout> <callout arearefs="nfe.modelname">Determine Model
		 * Name</callout> <callout arearefs="nfe.modelreader">Read
		 * Model</callout> </calloutlist>
		 */
		// <end id="maxent.examples.namefinder.setup"/>
	}

	/**
	 * Extract the model name from the model file, this is used to display the
	 * type of named entity found
	 * 
	 * @param language
	 * @param modelFile
	 * @return
