...
/**
 * General-purpose driver class for Taming Text programs. Utilizes
 * org.apache.hadoop.util.ProgramDriver to run main methods of other classes,
 * but first loads up default properties from a properties file.
 *
...
public class TamingTextDriver {
	...
	private static final Logger log = LoggerFactory
			.getLogger(TamingTextDriver.class);
	...
	public static void main(String[] args) throws Throwable {
		try {
			...
			for (Object key : mainClasses.keySet()) {
				...
				if (args.length > 0
						&& shortName(mainClasses.getProperty(keyString))
								.equals(args[0])) {
					foundShortName = true;
				}
				addClass(programDriver, keyString,
						mainClasses.getProperty(keyString));
			}
			...
			if (defaultsStream != null) { // can't find props file, use empty
											// props.
			...
			while (i < args.length && args[i] != null) {
				...
				String arg = args[i];
				i++;
				if (arg.length() > 2 && arg.charAt(1) == 'D') { // '-Dkey=value'
																// or
																// '-Dkey=value1,value2,etc'
																// case
			...
			for (String key : mainProps.stringPropertyNames()) {
				...
				String longArg = argNamePair.length < 2 ? null : "--"
						+ argNamePair[1].trim();
				if (!argMap.containsKey(shortArg)
						&& (longArg == null || !argMap.containsKey(longArg))) {
					argMap.put(longArg,
							new String[] { mainProps.getProperty(key) });
				}
			}
			...
			for (String arg : argMap.keySet()) {
				if (arg.startsWith("-D")) { // arg is -Dkey - if value for this
											...
											// "=" + value
					if (argMap.get(arg).length > 0
							&& !argMap.get(arg)[0].trim().isEmpty()) {
			...
			log.error("TamingTextDriver failed with args: "
	...
	private static void addClass(ProgramDriver driver, String classString,
			String descString) {
		try {
