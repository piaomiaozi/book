...
public class StackOverflowStream implements Iterator<StackOverflowPost>,
		Iterable<StackOverflowPost> {
	private static final Logger log = LoggerFactory
			.getLogger(StackOverflowStream.class);
	...
	public StackOverflowStream(String inputFile) throws IOException,
			XMLStreamException {
	...
	protected void populatePost() {
		...
		for (int i = 0; i < attrCount; i++) {
			attributeToPostField(reader.getAttributeLocalName(i),
					reader.getAttributeValue(i), post);
		}
	}
		...
		for (String tag : tags) {
			...
			for (int i = 0; i < len; i++) {
				if (tagc[i] == '<') {
					...
					outputTags.add(String.valueOf(tagc, start + 1, i - start
							- 1));
				}
			}
		}
	...
	protected void attributeToPostField(String name, String value,
