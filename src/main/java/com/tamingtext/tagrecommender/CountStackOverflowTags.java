...
public class CountStackOverflowTags {
	...
	private static final Logger log = LoggerFactory
			.getLogger(CountStackOverflowTags.class);
	...
	public boolean parseArgs(String[] args) {
		...
		Option limitOpt = obuilder
				.withLongName("limit")
				.withRequired(false)
				.withArgument(
						abuilder.withName("limit").withMinimum(1)
								.withMaximum(1).create())
				.withDescription("Emit this many of the most frequent tags")
				.withShortName("l").create();

		Option cutoffOpt = obuilder
				.withLongName("cutoff")
				.withRequired(false)
				.withArgument(
						abuilder.withName("cutoff").withMinimum(1)
								.withMaximum(1).create())
				.withDescription("Drop tags with a count less than this number")
	...
	public void count() throws IOException, XMLStreamException {
		...
		int docCount = 0, tagCount = 0, uniqueCount = 0, pastCutoff = 0, ret = 0;
		...
		System.err.println("Processed " + docCount + " documents, " + tagCount
				+ " tags, " + uniqueCount + " unique tags");
		System.err.println(pastCutoff + " tags occur " + cutoff
				+ " or more times");
		...
		System.err.println("Least tag count "
				+ rankedTags[maxTags - 1].getCount());
