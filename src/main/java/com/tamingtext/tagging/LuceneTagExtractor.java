...
public class LuceneTagExtractor {
	...
	public static void main(String[] args) throws IOException {
		...
		Option inputOpt = obuilder
				...
				.withArgument(
						abuilder.withName("dir").withMinimum(1).withMaximum(1)
								.create())
				.withDescription("The Lucene directory").withShortName("d")
				.create();
		...
		Option maxOpt = obuilder
				...
				.withRequired(false)
				.withArgument(
						abuilder.withName("max").withMinimum(1).withMaximum(1)
								.create())
				.withDescription(
						"The maximum number of vectors to output.  If not specified, then it will loop over all docs")
				.withShortName("m").create();

		Option fieldOpt = obuilder
				.withLongName("field")
				.withRequired(true)
				.withArgument(
						abuilder.withName("field").withMinimum(1)
								.withMaximum(1).create())
				.withDescription("The field in the index").withShortName("f")
				.create();
		...
		Group group = gbuilder.withName("Options").withOption(inputOpt)
				.withOption(outputOpt).withOption(maxOpt).withOption(fieldOpt)
				.create();
		...
		try {
			...
			if (cmdLine.hasOption(outputOpt)) {
				out = new PrintWriter(new FileWriter(cmdLine
						.getValue(outputOpt).toString()));
			} else {
	...
	public static void dumpTags(File file, String field, long maxDocs)
			throws IOException {
	...
	public static void emitTextForTags(File file, File output)
			throws IOException {
		String field = "tag";
		...
		do {
			...
			if (te.docFreq() > 30) {
				...
				while (td.next()) {
					int doc = td.doc();
					buf.setLength(0);
					appendVectorTerms(buf, reader.getTermFreqVector(doc,
	...
	public static void emitTermsForTags(PrintWriter out, StringBuilder buf,
			IndexReader reader, TermFreqVector tv) {
		if (tv == null)
			return;
	...
	public static void dumpDocs(File indexDir, PrintWriter out, long maxDocs)
			throws IOException {
		...
		for (int i = 0; i < max; i++) {
			if (!reader.isDeleted(i)) {
				...
				appendVectorTerms(buf,
						reader.getTermFreqVector(i, "extended-clustering"));
				emitTermsForTags(out, buf, reader,
						reader.getTermFreqVector(i, "tag"));
			}
		}
	}
}
