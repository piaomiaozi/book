...
/**
 * The QuestionQParser takes in a natural language question and produces a
 * Lucene {@link org.apache.lucene.search.spans.SpanNearQuery}
 *
 */
public class QuestionQParser extends QParser implements QAParams {
	...
	public Query parse() throws ParseException {
		...
		// <start id="qqp.parse"/>
		Parse parse = ParserTool.parseLine(qstr, parser, 1)[0];// <co
																// id="qqp.parseLine"/>
		/*
		 * <calloutlist> <callout arearefs="qqp.parseLine"><para>Parse the
		 * question using the <classname>TreebankParser</classname>. The
		 * resulting <classname>Parse</classname> object can then be utilized by
		 * the classifier to determine the Answer Type.</para></callout>
		 * </calloutlist>
		 */
		// <end id="qqp.parse"/>
		...
		try {
			...
			TokenStream ts = analyzer
					.tokenStream(field, new StringReader(qstr));
			while (ts.incrementToken()) {// <co id="qqp.addTerms"/>
				String term = ((CharTermAttribute) ts
		...
		return new SpanNearQuery(sql.toArray(new SpanQuery[sql.size()]),
				params.getInt(QAParams.SLOP, 10), true);// <co
														// id="qqp.spanNear"/>
		/*
		 * <calloutlist> <callout arearefs="qqp.handleAT"><para>Add the
		 * AnswerType to the query</para></callout> <callout
		 * arearefs="qqp.addTerms"><para>Add the original query terms to the
		 * query</para></callout> <callout arearefs="qqp.spanNear"><para>Query
		 * the index looking for all of the parts near each
		 * other</para></callout> </calloutlist>
		 */
		// <end id="qqp.query"/>
	}
