...
public class ChunkParser implements Parser {
	...
	public Parse parse(Parse tokens) {
		...
		for (int j = 0; j < words.length; j++) {
			...
			double prob = probs[j];
			tokens.insert(new Parse(word.getText(), word.getSpan(), tags[j],
					prob, j));// <co id="cp.augment"/>
			tokens.addProb(Math.log(prob));
		}
		/*
		 * <calloutlist> <callout arearefs="cp.child"><para>The
		 * <methodname>parse</methodname> is a callback method from an internal
		 * OpenNLP API that tokenizes the original text.</para></callout>
		 * <callout arearefs="cp.words"><para>Get just the words for use with
		 * the tagger</para></callout> <callout arearefs="cp.tag"><para>Part of
		 * speech tag the words</para></callout> <callout
		 * arearefs="cp.probs"><para></para></callout> <callout
		 * arearefs="cp.augment"><para>Augment the initial parse with the part
		 * of speech information</para></callout> </calloutlist>
		 */
		// <end id="cp.pos"/>
		...
		for (int ci = 0, cn = chunks.length; ci < cn; ci++) {
			if (ci > 0 && !chunks[ci].startsWith("I-")
					&& !chunks[ci - 1].equals("O")) {
				Span span = new Span(children[chunkStart].getSpan().getStart(),
						children[ci - 1].getSpan().getEnd());
				tokens.insert(new Parse(tokens.getText(), span, chunkType,
						logProb, children[ci - 1]));
				logProb = 0;
			}
		...
		if (!chunks[chunks.length - 1].equals("O")) {
			...
			Span span = new Span(children[chunkStart].getSpan().getStart(),
					children[ci - 1].getSpan().getEnd());
			tokens.insert(new Parse(tokens.getText(), span, chunkType, logProb,
					children[ci - 1]));
		}
		return tokens;
	}
