...
public class WikipediaIndexer {
	...
	private static SimpleDateFormat formatter = new SimpleDateFormat(
			"MM/dd/yyyy");
	private static SimpleDateFormat solrFormatter = new SimpleDateFormat(
	...
	public int index(File wikipediaXML, int numDocs, int batchSize)
			throws Exception {
		int result = 0;
	...
	public static void main(String[] args) throws Exception {
		...
		Group group = gbuilder.withName("Options").withOption(wikipediaFileOpt)
				...
				.withOption(solrBatchOpt).create();
		...
		WikipediaIndexer indexer = new WikipediaIndexer(
		...
		for (int i = 0; i < dumpFiles.length && total < numDocs; i++) {
			...
			long finish = System.currentTimeMillis();
			if (log.isInfoEnabled()) {
				log.info("Indexing " + dumpFile + " took " + (finish - start)
						+ " ms");
			}
			total += totalFile;
