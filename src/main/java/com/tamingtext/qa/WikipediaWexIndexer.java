...
public class WikipediaWexIndexer {
	private transient static Logger log = LoggerFactory
			.getLogger(WikipediaWexIndexer.class);
		...
		if (wikipediaWEX != null && wikipediaWEX.isFile()) {
			...
			try {
				...
				while ((docData = contentSource.getNextDocData(docData)) != null
					...
					String[] categories = docData.getProps()
							.getProperty("category").split(";;");
	...
	public static void main(String[] args) throws Exception {
		...
		Option wikipediaFileOpt = obuilder
				...
				.withRequired(true)
				.withArgument(
						abuilder.withName("wikiFile").withMinimum(1)
		...
		Option numDocsOpt = obuilder
				...
				.withRequired(false)
				.withArgument(
						abuilder.withName("numDocs").withMinimum(1)
								.withMaximum(1).create())
				.withDescription("The number of docs to index")
		...
		Option solrBatchOpt = obuilder
				...
				.withRequired(false)
				.withArgument(
						abuilder.withName("batch").withMinimum(1)
								.withMaximum(1).create())
				.withDescription(
						"The number of docs to include in each indexing batch")
		...
		Group group = gbuilder.withName("Options").withOption(wikipediaFileOpt)
				.withOption(numDocsOpt).withOption(solrURLOpt)
				.withOption(solrBatchOpt).withOption(helpOpt).create();
		...
		try {
			...
			if (cmdLine.hasOption(numDocsOpt)) {
				numDocs = Integer.parseInt(cmdLine.getValue(numDocsOpt)
						.toString());
			}
			...
			if (cmdLine.hasOption(solrBatchOpt)) {
				batch = Integer.parseInt(cmdLine.getValue(solrBatchOpt)
						.toString());
			}
			...
			for (int i = 0; i < dumpFiles.length && total < numDocs; i++) {
				...
				long finish = System.currentTimeMillis();
				if (log.isInfoEnabled()) {
					log.info("Indexing " + dumpFile + " took "
							+ (finish - start) + " ms");
				}
				total += totalFile;
